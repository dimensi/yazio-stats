    ...
    .action(async (opts: CommonOptions & { date?: string }, command: Command) => {
      ...
      try {
        const [summary, items, weightResp, goals] = await Promise.all([
          ...
          client.user.getWeight({ date }) as Promise<{ value: number | null; date: string } | null>,
        ...
        const weightKg = weightResp?.value ?? summary?.user?.current_weight ?? null;
        ...
        for (const item of products) {
          if (!productNames.has(item.product_id)) {
            try {
              const product = await client.products.get(item.product_id) as any;
              productNames.set(item.product_id, product?.name ?? item.product_id);
            } catch {
        ...
        const byMeal: Record<string, string[]> = { breakfast: [], lunch: [], dinner: [], snack: [] };
        for (const item of products) {
        ...
        for (const item of (items?.simple_products ?? []) as any[]) {
          const name = item.name ?? "—";
          byMeal[item.daytime].push(`${name} (-г)`);
        }
        for (const item of (items?.recipe_portions ?? []) as any[]) {
          const name = (item as any).name ?? "—";
          const daytime = (item as any).daytime;
          if (daytime && byMeal[daytime]) byMeal[daytime].push(`${name} (-г)`);
        }
            ...
            JSON.stringify(
              {
                ...
                calories: { value: calories, goal: goalCal, pct: pct(calories, goalCal) },
                protein: { value: protein, goal: goalProtein, pct: pct(protein, goalProtein) },
                carbs: { value: carbs, goal: goalCarbs, pct: pct(carbs, goalCarbs) },
        ...
        console.log(`🔥 Калории: ${calories} ккал (${pct(calories, goalCal)})`);
        console.log(`🍗 Белки: ${protein.toFixed(1)} г (${pct(protein, goalProtein)})`);
        console.log(`🍞 Углеводы: ${carbs.toFixed(1)} г (${pct(carbs, goalCarbs)})`);
        console.log(`🥑 Жиры: ${fat.toFixed(1)} г (${pct(fat, goalFat)})`);
        console.log(`👟 Шаги: ${steps} шагов`);
        if (weightKg != null) {
          const w = Number(weightKg).toFixed(1);
          const goalWeightLine = goalWeight != null ? ` (цель: ${Number(goalWeight).toFixed(1)} кг)` : "";
